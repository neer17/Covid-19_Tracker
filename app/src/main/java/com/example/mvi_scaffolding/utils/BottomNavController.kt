...
class BottomNavController(
    ...
    @IdRes val appStartDestinationId: Int,  //  id of graph
    val graphChangeListener: OnNavigationGraphChanged?,
    val navGraphProvider: NavGraphProvider
) {
    private val TAG = "AppDebug: " + BottomNavController::class.java.simpleName
    ...
    fun onNavigationItemSelected(itemId: Int = navigationBackStack.last()): Boolean {
        ...
        // For each bottom navigation item a new NavHostFragment would be created
    ...
    //  handle the back stack of items in BottomNavigation
    ...
    // Get id of each graph and return navigation id
    ...
    //  initialization of "OnNavigationItemChanged"
...
// Convenience extension to set up the navigation inside the NavHostFragment
fun BottomNavigationView.setUpNavigation(
    bottomNavController: BottomNavController,
    onReselectListener: BottomNavController.OnNavigationReselectedListener
) {
    val TAG = "BottomNavigationView"

    /* creates or find a NavHostFragment and add it to the last in the "navigationBackStack" */
    setOnNavigationItemSelectedListener {
        Log.d(TAG, "setUpNavigation: menu item: $it")
    ...
    setOnNavigationItemReselectedListener {
        bottomNavController
            .fragmentManager
            .findFragmentById(bottomNavController.containerId)!!    //  this wil return the NavHostFragment of each item
            .childFragmentManager.fragments.size.let {
                Log.d(TAG, "setUpNavigation: total fragments: $it")
        ...
        bottomNavController
            .fragmentManager
            .findFragmentById(bottomNavController.containerId)!!    //  this wil return the NavHostFragment of each item
            .childFragmentManager   //  this will be having only one fragment as only one fragment is shown
            //  as it has not idea about fragments in the nav graph
            .fragments[0]?.let { fragment ->
            onReselectListener.onReselectNavItem(
                bottomNavController.activity.findNavController(bottomNavController.containerId), // getting nav controller of view group Frame Layout
                fragment
            )
        }
    }
